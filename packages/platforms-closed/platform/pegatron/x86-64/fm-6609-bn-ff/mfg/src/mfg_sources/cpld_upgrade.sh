...
function Cpld_Upgrade()
{
    ...
    if [[ "$PROJECT_NAME" == "BUGATTI" ]] && [[ "$board_sel" == "mb" ]]; then
        ...
        Write_I2C_Device_Node $I2C_BUS $I2C_MUX_B $I2C_MUX_REG $I2C_MUX_B_CHANNEL_0
        orig_data=$( { Read_I2C_Device_Node $I2C_BUS $CPLD_A_ADDR $CPLD_MCR_REG ; } 2>&1 )
        data_cpu_en=$(( $orig_data | 0x20 ))                          # JTAG_EN_NPU_R [5] 0:header ; 1:CPU
        Write_I2C_Device_Node $I2C_BUS $CPLD_A_ADDR $CPLD_MCR_REG $data_cpu_en
    ...
    if [[ "$SUPPORT_CPU" == "RANGELEY" ]]; then
        ...
        if [[ "$board_sel" == "npu" ]]; then
            ...
            orig_bmcr1_data=$( { Read_I2C_Device_Node $I2C_BUS $NPU_CONTROL_CHIP_ADDR $CPLD_D_BMCR1_REG ; } 2>&1 )
            enable_cpu_jtag=$(( $orig_bmcr1_data | 0x01 ))
            Write_I2C_Device_Node $I2C_BUS $NPU_CONTROL_CHIP_ADDR $CPLD_D_BMCR1_REG $enable_cpu_jtag
    ...
    if [[ "$PROJECT_NAME" == "BUGATTI" ]] && [[ "$board_sel" == "mb" ]]; then
        ...
        Write_I2C_Device_Node $I2C_BUS $CPLD_A_ADDR $CPLD_MCR_REG $orig_data
    fi

    ## close GPIO file node , due to Denverton GPIO setting is wrapped in the excusion file so don't need additional calling here.
    if [[ "$SUPPORT_CPU" != "DENVERTON" ]]; then
        ## This is for BDX with BMC layout
        Write_I2C_Device_Node $I2C_BUS $NPU_CONTROL_CHIP_ADDR $CPLD_D_BMCR1_REG $orig_bmcr1_data
