...
{
    model_name=$( { onlpdump -o | grep "Product Name:" | rev | cut -d ' ' -f 1 | rev ; } 2>&1 )
...
function Get_HW_Info()
{
    ...
    if [[ "$SUPPORT_CPU" == "BDXDE" ]];then
        ## CPLD
        data_result=$( { Read_I2C_Device_Node $I2C_BUS_ARBITER_AND_AFTER $NPU_CONTROL_CHIP_ADDR $CPLD_VER_REG; } 2>&1 )
        ...
        if (( 0 )); then    ## This case is for old (short) BDX only, because old CPLD NOT detect HW revision pin ...
            ...
            if [[ "$board_revision_bit" == "0" ]]; then     #000
                ...
                board_revision="BDX-DE-BMC_NPU REV. 2.00"
            else
                board_revision="??"
            fi
        ...
        ## MCU
        data_result=$( { Read_I2C_Device_Node $I2C_BUS_NPU $NPU_CONTROL_CHIP_ADDR 0x1 ; } 2>&1 )
    ...
    ## MB HW version - by CPLD "B" to detect version
    data_result=$( { Read_I2C_Device_Node $I2C_BUS_MUX_B_CHANNEL_1 $CPLD_B_ADDR $CPLD_VER_REG ; } 2>&1 )
    hw_ver=$(($data_result >> 5))
    case $hw_ver in
    ...
    data_result=$( { Read_I2C_Device_Node $I2C_BUS_MCU $MB_MCU_ADDR $FB_MCU_HW_VERSION_REG; } 2>&1 )
    data_expander=$( { Read_I2C_Device_Node $I2C_BUS_MCU $MB_MCU_ADDR $MB_MCU_I2C_BUS_ALERT_REG ; } 2>&1 )
    ...
    if (( $check_ioexpander_alert == 1 )); then    # io_expander (0x3a) NOT exist
        ...
        sysVer=$( { Read_I2C_Device_Node $I2C_BUS_MCU $MB_MCU_ADDR 0x6 ; } 2>&1 )
        if [[ "$sysVer" == "0x00" ]]; then
            SYS_id="5X40MM_FC_DB"
        elif [[ "$sysVer" == "0x01" ]]; then
            SYS_id="5X40MM_FC_DB"              ## for FHGW, max 3pcs Fan modules
...
{
    linux_ver=$( cat /proc/version | cut -c15-39 )
    printf '  Linux: '
    echo $linux_ver
}
...
function Get_SDK_FW_Version()
{
    if [[ "$PROJECT_NAME" == "BUGATTI" ]]; then
        ...
        echo "  MFG: Aston with Broadwell-DE w/ BMC --- v0.0.4"
        echo "  SDK: nps_system_v3.0.4"
...
function Get_CPLD_FW_Version()
{
    ...
    # CPLD C
    if [[ "$PROJECT_NAME" != "FHGW" ]]; then
        data_result=$( { Read_I2C_Device_Node $I2C_BUS_MUX_B_CHANNEL_2 $CPLD_C_ADDR $CPLD_VER_REG; } 2>&1 )
        fw_ver=$(($data_result & 0x1f))
        ## bit-flag to declare verified or formal release version, by EE declared.
        if (( ( $data_result & 0x10 ) >> 4 == 1 )); then
            fw_ver=$(($data_result & 0x0f))
...
function Get_MCU_FW_Version()
{
    ...
    do
        ...
        data_result=$( { Read_I2C_Device_Node $I2C_BUS_MCU $MB_MCU_ADDR $mcu_index; } 2>&1 )
        mainVer=$(($data_result >> 4))
        subVer=$(($data_result & 0x0f))

        board_index=$(($MB_MCU_HW_VERSION_REG | $mcu))
        data_result=$( { Read_I2C_Device_Node $I2C_BUS_MCU $MB_MCU_ADDR $board_index; } 2>&1 )
        ...
        if (( $mcu == 0x0 )); then
            if [[ "$board_ID_bit" == "11" ]]; then   #00011
                ...
                board_ID="GEMINI"
            elif [[ "$board_ID_bit" == "1010" ]]; then   #01010
                board_ID="FHGW"
            else
                board_ID="??"
            fi
...
function Get_PMBus_Version()
{
    ...
    data=$( { i2cdump -y $I2C_BUS_ARBITER_AND_AFTER $PMBUS_NPU_63_ADDR s $PMBUS_NVM_CHECKSUM_REG ; } 2>&1 )
    ...
    data=$( { i2cdump -y $I2C_BUS_ARBITER_AND_AFTER $PMBUS_NPU_64_ADDR s $PMBUS_NVM_CHECKSUM_REG ; } 2>&1 )
    for (( i = 0 , j = 2 ; i < 4 ; i++, j=i+2 ))
    do
        array_pmbus[i]=$( { echo $data | cut -d ':' -f 2 | cut -d ' ' -f $j ; } 2>&1 )
    done
    ...
    if [[ "$PROJECT_NAME" == "BUGATTI" ]]; then    ## (0xf of CPLD-A)
        Write_I2C_Device_Node $I2C_BUS_MUX_B_CHANNEL_0 $CPLD_A_ADDR 0xf 0x1
    ...
    if [[ "$PROJECT_NAME" == "ASTON" ]]; then
        ## Get firmware checksum of multiphase controller TPS536C7 on main board.
        ## Multiphase Controller(A) : 0x66
        fw_checksum=$( { Read_I2C_Device_Node $I2C_BUS_PMBUS $PMBUS_MB_A_ADDR $MPC_TPS536C7_NVM_CHECKSUM_REG w ; } 2>&1 )
        echo "  PMBus FW checksum (MB  $PMBUS_MB_A_ADDR) : $fw_checksum"

        ## Multiphase Controller(B) : 0x6A
        fw_checksum=$( { Read_I2C_Device_Node $I2C_BUS_PMBUS $PMBUS_MB_B_ADDR $MPC_TPS536C7_NVM_CHECKSUM_REG w ; } 2>&1 )
        echo "  PMBus FW checksum (MB  $PMBUS_MB_B_ADDR) : $fw_checksum"
    else
        ## Get firmware checksum of multiphase controller TPS53679 on main board.
        data=$( { i2cdump -y $I2C_BUS_PMBUS $PMBUS_MB_A_ADDR s $PMBUS_NVM_CHECKSUM_REG ; } 2>&1 )
        for (( i = 0 , j = 2 ; i < 4 ; i++, j=i+2 ))
        do
            array_pmbus[i]=$( { echo $data | cut -d ':' -f 2 | cut -d ' ' -f $j ; } 2>&1 )
        done
        ...
        if [[ "$PROJECT_NAME" == "GEMINI" ]]; then
            data=$( { i2cdump -y $I2C_BUS_PMBUS $PMBUS_MB_C_ADDR s $PMBUS_NVM_CHECKSUM_REG ; } 2>&1 )
            for (( i = 0 , j = 2 ; i < 4 ; i++, j=i+2 ))
            do
                array_pmbus[i]=$( { echo $data | cut -d ':' -f 2 | cut -d ' ' -f $j ; } 2>&1 )
            done
    ...
    if [[ "$PROJECT_NAME" == "BUGATTI" ]]; then
        Write_I2C_Device_Node $I2C_BUS_MUX_B_CHANNEL_0 $CPLD_A_ADDR 0xf 0x0
    elif [[ "$PROJECT_NAME" == "JAGUAR" ]]; then
