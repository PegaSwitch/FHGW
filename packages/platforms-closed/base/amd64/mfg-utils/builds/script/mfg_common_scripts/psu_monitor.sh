...
function Get_PSU_Data ()    # $1 = $psu_addr ; $2 = $psu_reg
{
    ...
    if (( $psu_reg == PSU_VIN_REG )); then
        ...
        if [ `echo "$P_out > 0"| bc` -gt 0 ]; then   ## 2020/02/06 julia modified, for ${P_out}=0.350 cannot compare bug
            flag_detect=1
        fi
...
if [[ "$PROJECT_NAME" == "ASTON" ]]; then
    printf "\t{ PSU-2 Information }\n"
else
    printf "\t{ PSU-A Information }\n"
fi
I2C_Device_Detect $I2C_BUS_PSU_A $PSU_A_ADDR
if [[ "$device_exist_check" == "$FAIL" ]]; then
    if [[ "$PROJECT_NAME" == "ASTON" ]]; then
        printf "\t # PSU-2 not exist , Skip detect data\n\n"
    else
        printf "\t # PSU-A not exist , Skip detect data\n\n"
    fi
else
    # Print PSU Information
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_VIN_REG
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_VOUT_REG
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_IIN_REG
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_IOUT_REG
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_PIN_REG
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_POUT_REG
    Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_FAN_SPEED_REG
    if (( $flag_detect == 1 )); then
        Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_TEMP_1_REG
        Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_TEMP_2_REG
        Get_PSU_Data $I2C_BUS_PSU_A $PSU_A_ADDR $PSU_TEMP_3_REG
        flag_detect=0
    else
...
if [[ "$PROJECT_NAME" == "ASTON" ]]; then
    printf "\t{ PSU-1 Information }\n"
else
    printf "\t{ PSU-B Information }\n"
fi

I2C_Device_Detect $I2C_BUS_PSU_B $PSU_B_ADDR
if [[ "$device_exist_check" == "$FAIL" ]]; then
    if [[ "$PROJECT_NAME" == "ASTON" ]]; then
        printf "\t # PSU-1 not exist , Skip detect data\n\n"
    else
        printf "\t # PSU-B not exist , Skip detect data\n\n"
    fi
else
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_VIN_REG
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_VOUT_REG
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_IIN_REG
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_IOUT_REG
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_PIN_REG
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_POUT_REG
    Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_FAN_SPEED_REG
    if (( $flag_detect == 1 )); then
        Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_TEMP_1_REG
        Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_TEMP_2_REG
        Get_PSU_Data $I2C_BUS_PSU_B $PSU_B_ADDR $PSU_TEMP_3_REG
        flag_detect=0
    else
        printf "\tSkip detect Temperature data\n"
    fi
