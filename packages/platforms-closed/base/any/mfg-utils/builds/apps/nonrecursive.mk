...
ifeq ($(LIBRARY),shared)
  ifeq ($(OPSYS),linux)
    ...
    ifneq ($(filter $(CONFIG_FAMILY),rtl8380 alleycat3),)
    #FIXME: realtek compiler is too old :-(
      shared_cmds	= ln -sf $1 $$(call soname,$1)
    else
      shared_cmds	= ldconfig -nv . >/dev/null
    endif
  else
    LIB_EXT		:= dylib
...
quiet_cmd_mkdep_cc	?= DEP     $(subst ./,,$(patsubst $(srctree)/%,%,$<))
      cmd_mkdep_cc 	?= set -e; mkdir -p $(DEPEND_DIR);		\
	ext=`if [ "$(USE_HOSTTOOLS)" = "yes" ]; then			\
...
quiet_cmd_cc		?= CC      $(subst ./,,$(patsubst $(srctree)/%,%,$<))
      cmd_cc		?= set -e; mkdir -p $(OBJ_DIR);			\
	$(CC) -c $(INCLUDES) $(CPPFLAGS) $(CFLAGS) -o $@ $< 

quiet_cmd_cc_d		?= CC      $(subst ./,,$(patsubst $(srctree)/%,%,$<)) (debug) 
...
quiet_cmd_app		?= LD      $$@
      cmd_app		?= set -e; mkdir -p $(OBJ_DIR);			\
	$(CC) $(CFLAGS) -o $$@ -Wl,--start-group $$(filter-out %.$(LIB_EXT),$$^) $$(call xfer-libs,$(OBJ_DIR),$$^,$(LIB_EXT)) $$(call run-pkg-config,--libs,$$($$(notdir $$@)_requires)) $(filter -l%,$(LDFLAGS)) -Wl,--end-group

quiet_cmd_appd		?= LD      $$@ (debug)
      cmd_appd		?= set -e; mkdir -p $(OBJ_DEBUG_DIR);		\
	$(CC) $(CFLAGS) $(CFLAGS_D) -o $$@ -Wl,--start-group $$(filter-out %.$(LIB_EXT),$$^) $$(call xfer-libs,$(OBJ_DEBUG_DIR),$$^,$(LIB_EXT)) $$(call run-pkg-config,--libs,$$($$(patsubst %d,%,$$(notdir $$@))_requires)) $(filter -l%,$(LDFLAGS)) -Wl,--end-group
endif
...
-include $(patsubst %,%/module.mk,$(modules-y))
...
exclude_targets		:= $(EXTRA_EXCLUDE_TARGETS) 			\
			   ...
			   clean distclean info.targets info.dump rdep_test help
...
ifeq ($(filter $(exclude_targets),$(MAKECMDGOALS)),)
  ifeq ($(USE_VERSION.H),yes)
    $(DEPEND_DIR)/$(patsubst %.c,%.$(DEPEND_EXT),$(word $(words $(srcs_all)),$(srcs_all))): $(version_h) 
  endif
  -include $(depends)
endif 
...
#rel: $(apps_all:%=$(OBJ_DIR)/%)
#debug: $(apps_all:%=$(OBJ_DEBUG_DIR)/%d)
profile:
