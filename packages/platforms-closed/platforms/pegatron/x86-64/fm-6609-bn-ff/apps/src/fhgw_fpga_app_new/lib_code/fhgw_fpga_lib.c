...
int8_t fpga_read_config_reg(uint32_t offset, uint32_t *value)
{
    ioctl_arg_t params;
	...
	if(ioctl(fd, FHGW_FPGA_CONFIG_READ, &params) < 0) {
		printf("Error in ioctl get");
        return FPGA_RET_FAILED;
	}

    printf("\n LIB DBG : Read  Offset : 0x%x Value : %d", params.offset, params.value);
...
int8_t fpga_write_config_reg(uint32_t offset, uint32_t value)
{
    ioctl_arg_t params;
    ...
    if(ioctl(fd, FHGW_FPGA_CONFIG_WRITE, &params) < 0) {
        printf("Error in ioctl get");
        return FPGA_RET_FAILED;
    }

    printf("\n LIB DBG : Write  Offset : 0x%x Value : %d", params.offset, params.value);
...
int16_t fpga_get_roe_packet_len(uint8_t portno)
{
    if(fpga_validate_input_params(FPGA_VALIDATE_PORTNUM, portno) != FPGA_RET_SUCCESS)
        return FPGA_RET_FAILED;
...
int32_t fpga_get_roe_acccept_time_window(uint8_t portno)
{
    if(fpga_validate_input_params(FPGA_VALIDATE_PORTNUM, portno) != FPGA_RET_SUCCESS)
        return FPGA_RET_FAILED;
