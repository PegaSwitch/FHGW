...

{
    ...
    # Delay Time
    CMD_DELAY_TIME=30000                       # (unit: usec)
    MCU_UPGRADE_DELAY_TIME=1                   # (unit: sec)
    MCU_ENTER_BOOTLOADER_DELAY_TIME=300000     # (unit: usec)
...
function HW_Reset_Pin_Control()
{
    ...
    case "$PROJECT_NAME" in
        "MERCEDES")
            mux_channel=$I2C_MUX_B_CHANNEL_1
            ...
            ;;
        "BUGATTI")
            mux_channel=$I2C_MUX_B_CHANNEL_0
    ...
    Write_I2C_Device_Node $I2C_BUS $I2C_MUX_B $I2C_MUX_REG $mux_channel
...
function CPU_UART_Setting()
{
    ...
    if [[ "$SUPPORT_CPU" == "RANGELEY" ]] || [[ "$SUPPORT_CPU" == "BDXDE" ]]; then
        if [[ "$SUPPORT_CPU" == "BDXDE" ]]; then
            # cd $MFG_WORK_DIR/ioport_utility/

            checkGPIOstatus=$( { $MFG_WORK_DIR/ioport_utility/inl --hex 0x504 ; } 2>&1 )
            gpio_dir_17=$(( ( 0x$checkGPIOstatus & ( 1 << 17 ) ) >> 17 ))
            checkGPIOstatus_2=$( { $MFG_WORK_DIR/ioport_utility/inl --hex 0x534 ; } 2>&1 )
            gpio_dir_54=$(( ( 0x$checkGPIOstatus_2 & ( 1 << 22 ) ) >> 22 ))

            if (( $gpio_dir_17 != 0 || $gpio_dir_54 != 0 )); then    ## all as output pin
                echo " Default GPIO configure is fail ( 0x504 = 0x$checkGPIOstatus ; 0x534 = 0x$checkGPIOstatus_2 ), please confirm procees before doing MCU FW upgrade !!!"
                exit 1
            else
...
function Resources_Setting_Restore ()
{
    ...
    if [[ "$PROJECT_NAME" == "PORSCHE" ]]; then
        # Switch MUX to CPLD-B (i2cset -y 1 0x73 0x0 0x5)
        ...
        Write_I2C_Device_Node $I2C_BUS $CPLD_B_ADDR $CPLD_B_POWER_CIRCUIT_REG 0x0
    fi

}

function MCU_Bootloader_Setting ()
{
    if (( $uart_sel == $MB_MCU_UART_SEL )); then
        ...
        if [[ "$_upgrade_method" == "sw" ]]; then
            ...
            Write_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_FW_UPGRADE_REG $MCU_FW_UPGRADE_ENABLE
        ...
        if [[ "$_upgrade_method" == "sw" ]]; then
            ...
            Write_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $FB_MCU_FW_UPGRADE_REG $MCU_FW_UPGRADE_ENABLE
        elif [[ "$_upgrade_method" == "hw" ]]; then
            printf "by Hardware Method\n"
            HW_Hitless_Pin_Control "low"
            HW_Reset_Pin_Control "reset"
    ...
    usleep $MCU_ENTER_BOOTLOADER_DELAY_TIME
}
...
function MCU_Bootloader_Check ()
{
    if [[ "$_board_type" == "mb" ]]; then
        ...
        i2c_result=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_ALERT_REG ; } 2>&1 )
...
function MCU_Firmware_Upgrade()
{
    ...
    if [[ "$PROJECT_NAME" == "PORSCHE" ]]; then
        ...
        Write_I2C_Device_Node $I2C_BUS $CPLD_B_ADDR $CPLD_B_POWER_CIRCUIT_REG 0x1

        # re-Switch MUX to Main Board MCU (i2cset -y 1 0x72 0x0 0x7)
        Write_I2C_Device_Node $I2C_BUS $I2C_MUX_A $I2C_MUX_REG $I2C_MUX_CHANNEL_MCU
    fi

    MCU_Bootloader_Setting
    ...
    if (( $uart_sel == $MB_MCU_UART_SEL )); then
        ...
        if [[ "$_upgrade_method" == "sw" ]]; then    # due to HW method will cause both MB/FB MCU into bootloader mode, and then MB MCU can't be access in that time.
            ...
            if [[ "$into_bootloader_status" == "yes" ]]; then
                ...
                MCU_Bootloader_Setting    # re-setting into bootloader again
                MCU_Bootloader_Check $_board_type
                if [[ "$into_bootloader_status" == "yes" ]]; then
...
{
    ## Switch MUX to CPLD-A just for doing CPLD I2C bus check first.
...
function Denverton_NPU_MCU_Upgrade()
{
    ...
    RES=`Read_I2C_Device_Node $I2C_BUS $NPU_CONTROL_CHIP_ADDR 0x41`
    sleep 1s
    ...
    do
        echo -n "."
        #echo "i2cset -f -y $I2C_BUS $NPU_CONTROL_CHIP_ADDR 0x42 $line i"
        CMD="i2cset -f -y $I2C_BUS $NPU_CONTROL_CHIP_ADDR 0x42 $line i"
        RES=`$CMD`
    done < tmp
    echo "Done."

    #Step 7: REG43_FILE_TRANSFER_OK
    RES=`Read_I2C_Device_Node $I2C_BUS $NPU_CONTROL_CHIP_ADDR 0x43`
    sleep 1s
...
function Error_Message_Handler()
{
    ...
    case $error_id in
        ...
        $ERR_MSG_DEVICE_NOT_DETECT)
            if [[ "$sel" == "mcu" ]]; then
                printf "[Error] Device Main Board MCU($MB_MCU_ADDR) NOT Detect on I2C Bus $I2C_BUS !\n"
                return
            elif [[ "$sel" == "cpld" ]]; then
                printf "[Error] Device CPLD($PEGA_CPLD_A_ADDR) NOT Detect on I2C Bus $I2C_BUS !\n"
            elif [[ "$sel" == "other" ]]; then
                printf "[Error] Device NOT Detect on I2C Bus $I2C_BUS !\n"
            fi
            ;;
    ...
    if (( $board_type_check == $FAIL )); then
        ...
        #Compare_Checksum $file_name
            ...
            if [[ "$upgrade_method_sel" == "sw" ]]; then
                ...
                Write_I2C_Device_Node $I2C_BUS $I2C_MUX_A $I2C_MUX_REG $I2C_MUX_CHANNEL_MCU
                I2C_Device_Detect $I2C_BUS $MB_MCU_ADDR
