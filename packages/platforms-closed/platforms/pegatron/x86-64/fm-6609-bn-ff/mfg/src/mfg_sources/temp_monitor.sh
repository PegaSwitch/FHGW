...
temp_npu=$( { Read_I2C_Device_Node $I2C_BUS $NPU_TEMPER_SENSOR_ADDR $TEMPER_SENSOR_REG ; } 2>&1 )
if [[ $temp_npu != "" ]]; then
    printf "\tLM75B[NPU (Addr:0x4A)]:\t%d degrees Celsius.\n" "$temp_npu"
fi

# Set I2C MUX_A to channel with MainBoard MCU first.
Write_I2C_Device_Node $I2C_BUS $I2C_MUX_A $I2C_MUX_REG $I2C_MUX_CHANNEL_MCU

temp_pcb=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_TEMPER_SENSOR_PCB_REG ; } 2>&1 )
if [[ $temp_pcb != "" ]]; then
    printf "\tLM75B[PCB (Addr:0x48)]:\t%d degrees Celsius.\n" "$temp_pcb"
fi

temp_mac=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_TEMPER_SENSOR_MAC_REG ; } 2>&1 )
if [[ $temp_mac != "" ]]; then
    printf "\tLM75B[PCB (Addr:0x49)]:\t%d degrees Celsius.\n" "$temp_mac"
fi

temp_fb=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_TEMPER_SENSOR_FB_REG ; } 2>&1 )
if [[ $temp_fb != "" ]]; then
