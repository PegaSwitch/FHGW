...
# Print Fan PWM
fan_pwm=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_FAN_PWM_REG ; } 2>&1 )
if [[ $fan_pwm != "" ]]; then
...
# Print Fan PWM Mode
fan_pwm_mode=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_SMARTFAN_ENABLE_BASE_REG ; } 2>&1 )
...
# Print Alert Mode
fan_alert_mode=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $MB_MCU_FAN_ALERT_MODE_REG ; } 2>&1 )
...
do
    ...
    fan_inner_rpm=$( { Read_I2C_Device_Node_Word $I2C_BUS $MB_MCU_ADDR $fan_func_sel ; } 2>&1 )
...
do
    ...
    fan_outer_rpm=$( { Read_I2C_Device_Node_Word $I2C_BUS $MB_MCU_ADDR $fan_func_sel ; } 2>&1 )
...
do
    ...
    fan_status=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $fan_func_sel ; } 2>&1 )
    if [[ $fan_status != "" ]]; then
        printf "\n\t[Fan %d]     " $(( $fan + 1 ))
...
do
    ...
    fan_alert=$( { Read_I2C_Device_Node $I2C_BUS $MB_MCU_ADDR $fan_func_sel ; } 2>&1 )
    if [[ $fan_alert != "" ]]; then
        printf "\n\t[Fan %d]     " $(( $fan + 1 ))
...
Write_I2C_Device_Node $I2C_BUS $I2C_MUX_A $I2C_MUX_REG 0x0   # restore channel to default (no open channel)
