...
function MultiPhase_Controller_TPS40428_Monitor()
{
    ...
    # Switch to Channel-A Page
    Write_I2C_Device_Node $I2C_BUS $dev_addr 0x0 0x0

    # Read V_OUT (Channel A)
    vout_a_mantissa=$( { Read_I2C_Device_Node_Word $I2C_BUS $dev_addr 0x8B ; } 2>&1 )
    ...
    iout_a=$( { Read_I2C_Device_Node_Word $I2C_BUS $dev_addr 0x8C ; } 2>&1 )
    ...
    if (( 0 )); then    # Bugatti2/Porsche2/Mercedes3 didn't use channel-B
        # Switch to Channel-B Page
        Write_I2C_Device_Node $I2C_BUS $dev_addr 0x0 0x1

        # Read V_OUT (Channel B)
        vout_b_mantissa=$( { Read_I2C_Device_Node_Word $I2C_BUS $dev_addr 0x8B ; } 2>&1 )
        ...
        iout_b=$( { Read_I2C_Device_Node_Word $I2C_BUS $dev_addr 0x8C ; } 2>&1 )
    ...
    Write_I2C_Device_Node $I2C_BUS $dev_addr 0x0 0x81

    # Read Temperature_2 (Simultaneous Channel A and B)
    temp_2=$( { Read_I2C_Device_Node_Word $I2C_BUS $dev_addr 0x8E ; } 2>&1 )
...
function I2C_Arbiter_Request()
{
    ...
    if (( $request == 1 )); then
        ...
        i2cset -y $I2C_BUS $I2C_ARBITER_ADDR $I2C_ARBITER_CTRL_REG 0x1
        usleep $I2C_ACTION_DELAY
        ...
        i2cset -y $I2C_BUS $I2C_ARBITER_ADDR $I2C_ARBITER_CTRL_REG 0x0
        usleep $I2C_ACTION_DELAY
    fi
}
...
if [[ "$PROJECT_NAME" == "JAGUAR" ]]; then
    # Switch to CPLD-B to Control PSMCR
    ...
    # Switch to CPLD-A to Control PSMCR
    i2cset -y $I2C_BUS $I2C_MUX_B $I2C_MUX_REG $I2C_MUX_B_CHANNEL_0
    usleep $I2C_ACTION_DELAY

    # Set PWM SCL Mux Control Register(VRMCR), select to CPU
    Write_I2C_Device_Node $I2C_BUS $CPLD_A_ADDR 0x0F 0x1
fi

# Switch MUX to Channel to Access MB's Multiphase Controllers.
Write_I2C_Device_Node $I2C_BUS $I2C_MUX_PMBUS $I2C_MUX_REG $I2C_MUX_CHANNEL_PMBUS
...
if [[ "$PROJECT_NAME" == "JAGUAR" ]]; then
    ...
    i2cset -y $I2C_BUS $I2C_MUX_B $I2C_MUX_REG 0x0    # close channel
    usleep $I2C_ACTION_DELAY
elif [[ "$PROJECT_NAME" == "BUGATTI" ]]; then
    i2cset -y $I2C_BUS $I2C_MUX_B $I2C_MUX_REG $I2C_MUX_B_CHANNEL_0
    usleep $I2C_ACTION_DELAY
    Write_I2C_Device_Node $I2C_BUS $CPLD_A_ADDR 0x0F 0x0
    i2cset -y $I2C_BUS $I2C_MUX_B $I2C_MUX_REG 0x0    # close channel
    usleep $I2C_ACTION_DELAY
fi
